...
- A `sb_secret_...` key leaked into shell history during unrelated identity-bridge work on `robert-os-hub`/`Robert-OS` and was revoked before its dependents were inventoried. All three GitHub Actions bots share one repo secret, `SUPABASE_SERVICE_ROLE_KEY` -- the very next scheduled run failed: `strike_run.py` -> `HTTP Error 401: Unauthorized` on the first `strike_dca_settings` read
- **Dependency inventory done AFTER the fact, not before** (the actual mistake): grepped the whole repo for every `SUPABASE_SERVICE_ROLE_KEY` consumer. Three production files share one exact header pattern -- `strike_run.py` (2 call sites), `kraken_run.py` (2 call sites), `kraken_sync.py` (1 call site, via `kraken_run.SUPABASE_KEY` import) -- all sending the key in BOTH `apikey` and `Authorization: Bearer` headers
- **Root cause, once a fresh key was issued: the new key format itself, not just its value.** The old key was JWT-shaped; Supabase's current `sb_secret_...` keys are opaque, not a JWT, and current guidance sends them via `apikey` only. `Authorization: Bearer sb_secret_...` is not a valid use of the new format. Verified against the live project before touching any file: `apikey`-only returned `HTTP 200`, `Authorization: Bearer` with the new key did not
- **Fix: dropped the `Authorization` line at all five call sites across the three files.** Every other header, and the unrelated `STRIKE_API_KEY` `Authorization: Bearer` header in `strike_run.py` (a different service, Strike's own API, never affected), left untouched. `./test.sh` clean, 12/12 test files, before any live retest
- **A second, independent failure during the same recovery, worth recording so it is not mistaken for the same bug twice:** the first GitHub secret update silently stored an EMPTY value -- the job log showed `SUPABASE_URL: ***` (masked, present) beside `SUPABASE_SERVICE_ROLE_KEY:` with nothing after it. Most likely cause: the new key lived in one Termux shell's exported environment, and the `gh secret set` piping it in ran from a different session where that export never happened -- `echo ""` piped into `gh secret set` succeeds and stores an empty secret with no error. Second attempt verified the source variable was non-empty in the SAME command that piped it, not in a separate check beforehand
- **Verified against production, not just a compatibility probe -- one manual run per bot, all three:** Strike DCA (read), Kraken DCA (read), Kraken Sync (read AND write -- `sb_upsert` wrote `balances 8, open_orders 0, trades 1, ledgers 2` successfully). All three runs' checkout logs confirm they ran the fixed commit. No `401`, no `Unauthorized`, no `Traceback`, exit 0, in all three
- **`tools/kaina.py` still has the old dual-header pattern -- deliberately NOT fixed here.** It is not wired into any GitHub Actions workflow (confirmed by grep), so it was never part of this outage. Recorded as follow-up debt specifically so it is not later copied as a "known-working" reference for a new script, and so a future run of it against the current key does not produce a confusing, unrelated 401
- **For whoever reads this before touching these headers again: do not re-add `Authorization: Bearer` for an `sb_secret_...` key.** That combination is what caused this outage. `apikey` alone is correct for the current key format across all Supabase REST calls in this repo
...
- **Five of them were Lithuanian written WITHOUT diacritics** -- the specific thing Roberto has asked never to see. Two looked like internal DB strings and were not: `_mark_manual_required` puts its `note` argument straight into a Telegram message
- **Five contained an em dash**, against a standing rule to use the short dash
- `KRAKEN_ERROR_LT` -> `KRAKEN_ERROR_EN`, `kraken_error_lt` -> `kraken_error_en`. The name was the honest half of the change: a table called `_LT` returning English would be the next reader's trap
- **The real defect was not the violations, it was the coverage.** Both rules were already written down and one was already tested -- in exactly two messages out of twenty-six. Every test passed, every day, while most of the surface went unchecked. Same shape as the schema gate: the value is in covering the SET, not a well-chosen member of it
- `tests/test_message_charset.py` walks the AST of all three source files and checks every string literal, so text assigned to a variable and passed in later (`fail_action`, the `_mark_manual_required` notes) is covered rather than just call-site literals. Docstrings are exempt; `print` is exempt from the em-dash rule only, since the Actions log is not a phone
- **TWO layers, and the second one exists because of a mistake made writing the first.** The character-set check cannot see Lithuanian stripped of its diacritics -- which was five of the day's violations. The first draft of this file dropped the existing word-list check in favour of the charset scan and called it stronger; it was not stronger, it was DIFFERENT, and it left the hole exactly where the commonest defect lived. Both layers are now present and each was proved against a real reinstated violation: layer 1 reported only the em dash and said nothing about the two Lithuanian words beside it
- The scanner is itself tested for its ability to FAIL, and for having actually read every file it claims to cover. A checker that cannot fail reports safety it never measured, which this repo has shipped twice already
...
- **`Liko` now separates the two ideas instead of naming one and implying the other**: free cash and reserve, in one line. The `(N pirkimų)` count stays only in the no-hold case, where nothing needs explaining
- **The resting order is named with its distance from the market**, e.g. a buy limit sitting 3.5% below. Roberto's request, and it is the number that says whether the reserve is coming back today or is parked: the amount alone does not distinguish an order about to fill from one that has been out of reach for a week. The sign carries the direction, so no word is spent on it
- **Distance is measured per PAIR against a live ticker, not against the notification's own `Mid`.** That figure is the *reference* mid used for impact bps and can be the oldest reading in the run; measuring the gap with it would quietly answer a different question. Two USD-quoted orders on different pairs would likewise both be scored against whichever ticker was fetched last
- **Unknown distance prints nothing rather than `0.0%`.** A failed ticker read rendering as zero would announce that the order is about to execute, which is the opposite of not knowing
- Degrades the way the warning does: `OpenOrders` is a separate Kraken key permission from `Balance`, so a key that can see the amount and not the orders states the reserve and stops. The order lines are only requested when `hold_trade` says something is actually held -- no held money, no second API call
- **Labels are English, because every other label in this message already was.** `Liko` was the only Lithuanian word among Amount, Price, Cost, Fee and Total. Now `Balance: <x> available | <y> reserved` and `Limit Order: ...`, which is also the standard market terminology rather than a translation of it. The buy count stays Lithuanian: it is a sentence about the next purchase, not a field name
- **The order-type caption is READ from the order, not assumed.** Roberto asked for "Limit Order", and today it would be right by accident -- but `usd_holds` filters on side and quote currency and never on type, so a resting `stop-loss-limit` would have been captioned as something it is not. An unknown type is title-cased verbatim rather than mapped through a table, because a table nobody updates captions the next new type as a limit order
- Rendering split into pure `resting_order_line` / `distance_to_market` / `order_type_label`; 23 assertions on the live 08-04 fixture, including the missing-mid, the stop-order caption, and the `$60000` case that `:.5f` would have printed as `$60000.00000`
...
- **Worse than the wording: the number was unexplained.** Roberto sees $20 in the app and a warning saying $10, and nothing connects the two. The order is now named, so the difference is accounted for rather than argued with
- **The held money is stated as a fact, never as an instruction.** The first draft ended every held-money warning with "or cancel the order". Roberto killed it in one sentence: with enough for the buy there is nothing to fix, and a resting limit order is a deliberate position at a chosen price, not a mistake to undo. It now appears only when the next buy is **already** going to fail **and** the held amount would by itself cover it -- and even then as "jų pakaktų rytojaus pirkimui", not as an order to unwind. Same rule the Benas persona already carries: state the number, name the consequence, leave the decision
...
### feat(notify): a manual order that fills now says so -- `kraken_sync` v1.2.0, v1.2.1
- On 08-03 a limit order for the entire USDT balance filled at 02:44 and Roberto learned about it from a conversation at 07:30. The system announces its own DCA fills and said nothing about an order it did not place -- the mirror had the trade, and nothing read it out
- **Its own watermark (`trade_notify`), deliberately not the `trades` one.** That one is rewound by `WATERMARK_REWIND_MINUTES` on every read, so already-seen trades come back on purpose; correct for an upsert, and it would have re-sent the same message every run
- **The first run sends nothing.** With no watermark it would announce the account's whole history, which is how a new notification teaches its reader to mute it. It records the position and stays quiet
- DCA fills excluded by `order_txid` against both execution tables. The DCA already sends a better message -- impact, all-in bps, OHLC bands, none of which are known here. Grouped by order, because a partly filled limit arrives as several trades and three messages about one decision is noise
- **v1.2.1 adds the fee in the currency it was actually taken in.** `kraken_trades.fee` is always stated in the quote currency, which is a conversion rather than the event: that KASUSDT buy was charged in KAS, which is why the balance rose by less than the order volume, and that number appears nowhere in the trades table. Taken from the ledger row carrying a non-zero fee -- where `refid` is the TRADE id, not the order id, verified against a single order that produced two ledger rows sharing one refid. Shown in brackets only when it says something the dollar figure does not
- **Lag is real and not hidden:** with syncs at 08:00, 13:00 and 22:00 a fill can wait up to ten hours, so the message leads with the fill time rather than the word "now"
- **DECIDED AGAINST, so it is not reopened: a frequent `OpenOrders` poll.** Measured as cheap -- one API call, ~430 Actions minutes a month at 30-minute intervals, free on a public repo, and nowhere near Kraken's rate limit. Declined by Roberto on 2026-08-03 for the reason that settles it: he places few manual limit orders, so the feature would buy minutes of latency at the cost of 48-96 runs a day in the Actions log. It would also make defect 4c worse -- a silent failure hides as easily among 96 runs as among three
